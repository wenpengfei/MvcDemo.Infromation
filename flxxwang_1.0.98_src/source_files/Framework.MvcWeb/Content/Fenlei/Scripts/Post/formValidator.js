    ...
    $.formValidator =
{
        ...
        if (settings.formid != "") {
            $("#" + settings.formid).submit(function() {
                if ($.formValidator.pageIsValid("1")) {
                    if (settings.fun) {
                        if (settings.fun()) {
                            $.formValidator.subform(settings.formid, settings.btnid);
                        }
                        else {
                            return false;
                        }
    ...
    subform: function(fid, bid) {
        var form = $("#" + fid).get(0);
        var btn = $("#" + bid).get(0);
        //全局搜索文本
        try {
            var quanjufiled = "";
            $.each($("[f]"), function(j1, n1) {
                var f = $("#" + n1.id).attr("f");
                if (f) {
                    var v = $.j.getFullTextSearchVal(n1.id);
                    quanjufiled += f.replace(/\{0\}/g, v);
                }
            });
            if ($("#gobalsokey").val() == undefined) {
                $("#" + fid).append('<input name="gobalsokey" id="gobalsokey" type="hidden" value="' + quanjufiled + '"/>');
            }
            else {
                $("#gobalsokey").val(quanjufiled);
            }
            $.c.fk.s(fid);
        }
        catch (ex) { ; }
        try { $.j.createFieldForTextJoinValue(form); } catch (ex) { }; // 创建单行文本框拼接值控件
        try { $.c.Uploader.stopUploading(); } catch (e) { }; // 中止正在上传的图片
        // 发布
        var _url = window.location.href;
        _url = _url.replace(location.search, '');
        form.action = _url + "/submit";
        form.target = 'formSubmit';
        form.submit();
        btn.disabled = true;
        btn.value = '正在发布...';
    },
    subfalse: function(fid, bid) {
        var btn = $("#" + bid).get(0);
        btn.disabled = false;
        btn.value = '　　发布　　';
    },
    ...
    setTipState: function(elem, showclass, showmsg, tipid) {
        ...
        if (showmsg == null || showmsg == "") {
            ...
            if (showclass == "onError") {
                tip.html('<span class="c2"><span class="action_po"><span class="action_po_top">' + showmsg + '</span><span class="action_po_bot"></span></span></span>');
            }
            else if (showclass == "onCorrect") {
                tip.addClass("chenggong");
                tip.html('');
            }
                ...
                tip.addClass("action_po_top");
                tip.html('<span class="c1"><span class="action_po"><span class="action_po_top">' + showmsg + '</span><span class="action_po_bot"></span></span></span>');
            }
        }
    },
                    ...
                    if (!initConfig.alertmessage) {
                        if (error_tip.indexOf("^" + tipid + "^") == -1) {
                            if (!returnObj.isvalid) {
                                ...
                                $.formValidator.showMessage(returnObj); // 为解决ie6同行多控件验证不出提示的问题，调整了位置
                            }
                        }
                    }
                }
    ...
    checkContent: function(str) {
        var r = /[\u4e00-\u9fa5]/.test(str);
        if (!r) return '补充说明太过简单';
        return true;
    },
    // 不能填写特殊符号,只能填写汉字、数字、字母和.（）+ #
    checkSpecialSymbol: function(str) {
        var re = /^(([\u4e00-\u9fa5]|[0-9]|[a-zA-Z]|[.。()（）+#]))+$/;
        var r = re.test(str);
        if (!r) return '不允许填写特殊符号。只能填写：汉字、字母和.（）+ #';
        return r;
    },
    // 招聘类title检测
    checkZhaopinTitle: function(str) {
        var r = $.formValidator.IsSimple(str);
        if (!r) return '太过简单';
        r = $.formValidator.checkSpecialSymbol(str);
