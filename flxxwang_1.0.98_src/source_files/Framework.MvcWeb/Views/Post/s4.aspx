...
<head>
    ...
    %>
    <title>发布<%=infoCategory.Name %>信息-<%=infoCategory.Name %><%=SiteInfo.Name%></title>
...
<body>
    ...
    <%var region = this.ViewData["Region"] as Framework.Common.Region; %>
    <div id="header">
    ...
    <div class="flow_step_no4"> 
        ...
        <div class="flow_step"> 
            <ol class="cols5"> 
                ...
                <li class="step_3"><div><i>3</i><%=Html.ActionLink(region.Name, "s3", new { values = this.ViewData["ParentCode"] + "/" + this.ViewData["ChildCode"], tid = routeData.Values["tid"] })%><span></span></div></li>
    ...
    <div class="minheightout w">
        ...
        <div class="content minheight">
            <ul class="post4">
                <%foreach (var circle in (this.ViewData["Circles"] as IEnumerable<Framework.Common.Circle>))
                  { %>
                    <li><%=Html.ActionLink(circle.Name, "Post", this.ViewData["ParentCode"].ToString(), new { smallcategory = this.ViewData["ChildCode"], region = region.Id, circle = circle.Id, tid = routeData.Values["tid"] }, null)%></li>
                <%} %>
            </ul>
            <div class="c"></div>
        </div>
        <div class="hr_s"></div>
        <div id="pre-next">
            <%=Html.ActionLink("«重新选择区域", "s3", new { values = this.ViewData["ParentCode"] + "/" + this.ViewData["ChildCode"], tid = routeData.Values["tid"] })%>
            <%=Html.ActionLink("跳过，将信息发布到全" + region.Name + "»", "Post", this.ViewData["ParentCode"].ToString(), new { smallcategory = this.ViewData["ChildCode"], region = region.Id, tid = routeData.Values["tid"] }, null)%>
        </div>
    </div>

    <%--<script type="text/javascript">
        function goPostPage(a, b, c, d, e) {
            var url = "../" + a + "/" + b + "/?city=" + c + "&qy=" + d + "&qz=" + e + "";
            location.href = url;
        }
    </script>--%>
