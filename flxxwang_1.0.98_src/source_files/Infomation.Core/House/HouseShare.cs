...
namespace Infomation.Core
{
    ...
    /// 合租(单间出租).
    /// </summary>
    [Table(Name = "Info_HouseShare")]
    public class HouseShare : HouseBase<HouseShare>
    {
        public string Title
        {
            get;
        ...
        public override List<ObjectType> ObjectTypeCollection
        {
            get
            {
                if (_objectTypeCollection == null)
                {
                    ...
                    var category = houseCategory.Children.FirstOrDefault(item => item.Singleton == HouseShare.Singleton);
                    if (category != null)
                    {
        ...
        protected override DataAccess<HouseShare, int> GetProvider()
        {
            return new HouseShareDataAccess();
        }
