...
{
    public class CarFittingDataAccess : DataAccess<CarFitting, int>
    {
        //public override CarFitting Insert(CarFitting instance)
        //{
        //    CarFitting car = instance as CarFitting;

        //    string sql = "INSERT INTO " + TableName + " (Type,Title,ObjectType,Quality,Price,Tag1,Tag2,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source)" +
        //                                "VALUES (@Type,@Title,@ObjectType,@Quality,@Price,@Tag1,@Tag2,@Content,@IsBiz,@ContactPerson,@Phone,@QQOrMSN,@Pictures,@UserId,@CityId,@RegionId,@CircleId,@Source)";
        ...
        public override List<CarFitting> Insert(List<CarFitting> instances)
        {
            ...
            {
                sql += ";INSERT INTO " + TableName + " (Type,Title,ObjectType,Quality,Price,Tag1,Tag2,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source,Sequence,TId)" +
                                            "VALUES (@Type" + i + ",@Title" + i + ",@ObjectType" + i + ",@Quality" + i + ",@Price" + i + ",@Tag1_" + i + ",@Tag2_" + i + ",@Content" + i + ",@IsBiz" + i + ",@ContactPerson" + i + ",@Phone" + i + ",@QQOrMSN" + i + ",@Pictures" + i + ",@UserId" + i + ",@CityId" + i + ",@RegionId" + i + ",@CircleId" + i + ",@Source" + i + ",@Sequence"+i+",@TId"+i+")";
                sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";
            }
            sql = sql.Substring(1);
