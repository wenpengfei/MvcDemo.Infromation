...
{
    public class CarBicycleDataAccess : DataAccess<CarBicycle, int>
    {
        
        //public override CarBicycle Insert(CarBicycle instance)
        //{
        //    CarBicycle car = instance as CarBicycle;

        //    string sql = "INSERT INTO " + TableName + " (Type,Title,ObjectType,Quality,Price,Tag1,BrandId,BrandHandWrite,Size,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source)" +
        //                                "VALUES (@Type,@Title,@ObjectType,@Quality,@Price,@Tag1,@BrandId,@BrandHandWrite,@Size,@Content,@IsBiz,@ContactPerson,@Phone,@QQOrMSN,@Pictures,@UserId,@CityId,@RegionId,@CircleId,@Source)";
        ...
        //    db.AddInParameter(cmd, "@BrandHandWrite", DbType.String, car.BrandHandWrite);
        //    db.AddInParameter(cmd, "@Size", DbType.String, car.Size);
        ...
        public override List<CarBicycle> Insert(List<CarBicycle> instances)
        {
            ...
            {
                sql += ";INSERT INTO " + TableName + " (Type,Title,ObjectType,Quality,Price,Tag1,BrandId,BrandHandWrite,Size,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source,Sequence,TId)" +
                                             "VALUES (@Type" + i + ",@Title" + i + ",@ObjectType" + i + ",@Quality" + i + ",@Price" + i + ",@Tag1_" + i + ",@BrandId" + i + ",@BrandHandWrite" + i + ",@Size" + i + ",@Content" + i + ",@IsBiz" + i + ",@ContactPerson" + i + ",@Phone" + i + ",@QQOrMSN" + i + ",@Pictures" + i + ",@UserId" + i + ",@CityId" + i + ",@RegionId" + i + ",@CircleId" + i + ",@Source" + i + ",@Sequence"+i+",@TId"+i+")";
                sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";
            }
            sql = sql.Substring(1);
            ...
            for (var i = 0; i < instances.Count(); i++)
            {
                ...
                db.AddInParameter(cmd, "@BrandHandWrite" + i, DbType.String, car.BrandHandWrite);
                db.AddInParameter(cmd, "@Size" + i, DbType.String, car.Size);
