...
{
    public class CarAccompanyDataAccess : DataAccess<CarAccompany, int>
    {
        //public override CarAccompany Insert(CarAccompany instance)
        //{
        //    CarAccompany car = instance as CarAccompany;

        //    string sql = "INSERT INTO " + TableName + " (Title,ObjectType,Price,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source)" +
        //                                "VALUES (@Title,@ObjectType,@Price,@Content,@IsBiz,@ContactPerson,@Phone,@QQOrMSN,@Pictures,@UserId,@CityId,@RegionId,@CircleId,@Source)";
        ...
        public override List<CarAccompany> Insert(List<CarAccompany> instances)
        {
            ...
            {
                sql += ";INSERT INTO " + TableName + " (Title,ObjectType,Price,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source,Sequence,TId)" +
                                            "VALUES (@Title" + i + ",@ObjectType" + i + ",@Price" + i + ",@Content" + i + ",@IsBiz" + i + ",@ContactPerson" + i + ",@Phone" + i + ",@QQOrMSN" + i + ",@Pictures" + i + ",@UserId" + i + ",@CityId" + i + ",@RegionId" + i + ",@CircleId" + i + ",@Source" + i + ",@Sequence"+i+",@TId"+i+")";
                sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";
            }
            sql = sql.Substring(1);
