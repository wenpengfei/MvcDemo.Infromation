...
{
    public class CarBuyDataAccess : DataAccess<CarBuy, int>
    {
        //public override CarBuy Insert(CarBuy instance)
        //{
        //    CarBuy car = instance as CarBuy;

        //    string sql = "INSERT INTO " + TableName + " (Title,Tag1,Tag2,Exhaust,Transmission,Rundistance,Price1,Price2,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source)" +
        //                                "VALUES (@Title,@Tag1,@Tag2,@Exhaust,@Transmission,@Rundistance,@Price1,@Price2,@Content,@IsBiz,@ContactPerson,@Phone,@QQOrMSN,@Pictures,@UserId,@CityId,@RegionId,@CircleId,@Source)";
        ...
        //    db.AddInParameter(cmd, "@Rundistance", DbType.String, car.Rundistance);
        //    db.AddInParameter(cmd, "@Price1", DbType.Int32, car.Price1);
        //    db.AddInParameter(cmd, "@Price2", DbType.Int32, car.Price2);
        ...
        public override List<CarBuy> Insert(List<CarBuy> instances)
        {
            ...
            {
                sql += ";INSERT INTO " + TableName + " (Title,Tag1,Tag2,Exhaust,Transmission,Rundistance,Price1,Price2,[Content],IsBiz,ContactPerson,Phone,QQOrMSN,Pictures,UserId,CityId,RegionId,CircleId,Source,Sequence,TId)" +
                                            "VALUES (@Title" + i + ",@Tag1_" + i + ",@Tag2_" + i + ",@Exhaust" + i + ",@Transmission" + i + ",@Rundistance" + i + ",@Price1_" + i + ",@Price2_" + i + ",@Content" + i + ",@IsBiz" + i + ",@ContactPerson" + i + ",@Phone" + i + ",@QQOrMSN" + i + ",@Pictures" + i + ",@UserId" + i + ",@CityId" + i + ",@RegionId" + i + ",@CircleId" + i + ",@Source" + i + ",@Sequence"+i+",@TId"+i+")";
                sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";
            }
            sql = sql.Substring(1);
            ...
            for (var i = 0; i < instances.Count(); i++)
            {
                ...
                db.AddInParameter(cmd, "@Rundistance" + i, DbType.String, car.Rundistance);
                db.AddInParameter(cmd, "@Price1_" + i, DbType.Int32, car.Price1);
                db.AddInParameter(cmd, "@Price2_" + i, DbType.Int32, car.Price2);
