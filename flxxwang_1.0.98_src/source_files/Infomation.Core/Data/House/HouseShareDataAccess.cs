...
{
    public class HouseShareDataAccess : DataAccess<HouseShare, int>
    {
        //public override HouseShare Insert(HouseShare instance)
        //{
        //    HouseShare house = instance as HouseShare;

        //    string sql = "INSERT INTO " + TableName + " (Title,District,Address,Room,Parlor,Toilet,Floor,FloorTotal,Area,ObjectType,FitType,Toward,Price,PayWay,Bed,WaterHeater,WashingMachine,AirConditioner,Icebox,Television,Broadband,Bedroom,HireLimit,[Content],IsBiz,ContactPerson,Phone,Pictures,UserId,CityId,RegionId,CircleId,Source)" +
        //                                "VALUES (@Title,@District,@Address,@Room,@Parlor,@Toilet,@Floor,@FloorTotal,@Area,@ObjectType,@FitType,@Toward,@Price,@PayWay,@Bed,@WaterHeater,@WashingMachine,@AirConditioner,@Icebox,@Television,@Broadband,@Bedroom,@HireLimit,@Content,@IsBiz,@ContactPerson,@Phone,@Pictures,@UserId,@CityId,@RegionId,@CircleId,@Source)";
        //    sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";

        //    DbCommand cmd = db.GetSqlStringCommand(sql);
        //    //db.AddInParameter(cmd, "@HireType", DbType.Int32, house.HireType);
        ...
        public override List<HouseShare> Insert(List<HouseShare> instances)
        {
            ...
            {
                sql += ";INSERT INTO " + TableName + " (Title,District,Address,Room,Parlor,Toilet,Floor,FloorTotal,Area,ObjectType,FitType,Toward,Price,PayWay,Bed,WaterHeater,WashingMachine,AirConditioner,Icebox,Television,Broadband,Bedroom,HireLimit,[Content],IsBiz,ContactPerson,Phone,Pictures,UserId,CityId,RegionId,CircleId,Source,Sequence,TId)" +
                                            "VALUES (@Title" + i + ",@District" + i + ",@Address" + i + ",@Room" + i + ",@Parlor" + i + ",@Toilet" + i + ",@Floor" + i + ",@FloorTotal" + i + ",@Area" + i + ",@ObjectType" + i + ",@FitType" + i + ",@Toward" + i + ",@Price" + i + ",@PayWay" + i + ",@Bed" + i + ",@WaterHeater" + i + ",@WashingMachine" + i + ",@AirConditioner" + i + ",@Icebox" + i + ",@Television" + i + ",@Broadband" + i + ",@Bedroom" + i + ",@HireLimit" + i + ",@Content" + i + ",@IsBiz" + i + ",@ContactPerson" + i + ",@Phone" + i + ",@Pictures" + i + ",@UserId" + i + ",@CityId" + i + ",@RegionId" + i + ",@CircleId" + i + ",@Source" + i + ",@Sequence"+i+",@TId"+i+")";
                sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";
            }
            sql = sql.Substring(1);
            ...
            for (var i = 0; i < instances.Count(); i++)
            {
                ...
                db.AddInParameter(cmd, "@Bedroom" + i, DbType.String, house.Bedroom);
                db.AddInParameter(cmd, "@HireLimit" + i, DbType.String, house.HireLimit);
