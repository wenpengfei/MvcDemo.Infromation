...
{
    public class HouseBedDataAccess : DataAccess<HouseBed, int>
    {
        //public override HouseBed Insert(HouseBed instance)
        //{
        //    HouseBed house = instance as HouseBed;

        //    string sql = "INSERT INTO " + TableName + " (Title,District,Address,Room,Parlor,Toilet,Floor,FloorTotal,Area,ObjectType,FitType,Toward,Price,PayWay,Bed,WaterHeater,WashingMachine,AirConditioner,Icebox,Television,Broadband,HireLimit,[Content],IsBiz,ContactPerson,Phone,Pictures,UserId,CityId,RegionId,CircleId,Source)" +
        //                                "VALUES (@Title,@District,@Address,@Room,@Parlor,@Toilet,@Floor,@FloorTotal,@Area,@ObjectType,@FitType,@Toward,@Price,@PayWay,@Bed,@WaterHeater,@WashingMachine,@AirConditioner,@Icebox,@Television,@Broadband,@HireLimit,@Content,@IsBiz,@ContactPerson,@Phone,@Pictures,@UserId,@CityId,@RegionId,@CircleId,@Source)";
        ...
        public override List<HouseBed> Insert(List<HouseBed> instances)
        {
            ...
            {
                sql += ";INSERT INTO " + TableName + " (Title,District,Address,Room,Parlor,Toilet,Floor,FloorTotal,Area,ObjectType,FitType,Toward,Price,PayWay,Bed,WaterHeater,WashingMachine,AirConditioner,Icebox,Television,Broadband,HireLimit,[Content],IsBiz,ContactPerson,Phone,Pictures,UserId,CityId,RegionId,CircleId,Source,Sequence,TId)" +
                                             "VALUES (@Title" + i + ",@District" + i + ",@Address" + i + ",@Room" + i + ",@Parlor" + i + ",@Toilet" + i + ",@Floor" + i + ",@FloorTotal" + i + ",@Area" + i + ",@ObjectType" + i + ",@FitType" + i + ",@Toward" + i + ",@Price" + i + ",@PayWay" + i + ",@Bed" + i + ",@WaterHeater" + i + ",@WashingMachine" + i + ",@AirConditioner" + i + ",@Icebox" + i + ",@Television" + i + ",@Broadband" + i + ",@HireLimit" + i + ",@Content" + i + ",@IsBiz" + i + ",@ContactPerson" + i + ",@Phone" + i + ",@Pictures" + i + ",@UserId" + i + ",@CityId" + i + ",@RegionId" + i + ",@CircleId" + i + ",@Source" + i + ",@Sequence"+i+",@TId"+i+")";
                sql += " ;SELECT SCOPE_IDENTITY() as newIDValue ";
            }
            sql = sql.Substring(1);
